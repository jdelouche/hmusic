...
lully     p@(Glob [] tr ns) x    = mozart p x
lully     p x                    = sibellius p
beethoven p@(Glob tko tr ns) x   = let tko'=splitOn "," x in lully (Glob tko' tr ns) x
debussy   p "x"                  = vivaldi p
debussy   p x                    = arvopart p x
mozart p@(Glob tko tr ns) ""     = bach p
mozart p@(Glob tko tr ns) x      = debussy p x
wagner p@(Glob tko tr ns)        = do x<-getLine; beethoven p x
