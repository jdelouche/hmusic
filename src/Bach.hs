...
type Token     = String
data Data      = Line | Note
data Params    = Params { tokens :: Tokens, tracks :: Track Ticks, notes :: Notes } deriving (Eq,Show)
ravel        ::                                                 Midi -> IO ()
...
benevolo l xs ys      = do charpentier xs ; bach (Params l xs ys)
...
mozart    l xs ys x   = if (x/="") then debussy l xs ys x else bach (Params l xs ys)
wagner    l xs ys     = do x<-getLine; beethoven l xs ys x
bach (Params l xs ys) = if (length l == 0 ) then wagner l xs ys else let (z:zs) = l in mozart zs xs ys z
